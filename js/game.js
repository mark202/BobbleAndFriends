...
class Game {
  ...
  registerImpact(impact, weight = 1) {
    if (this.spawnGrace > 0) return;
    const strength = impact * weight;
    if (strength < 65) return;
    const shake = clamp(strength / 45, 1, 8);
    this.fx.addShake(shake);
    this.sound.play("hit");
  }
  ...
  handleBounds(friend) {
    ...
    if (friend.pos.x - radius < 0) {
      const impact = Math.abs(friend.vel.x);
      friend.pos.x = radius;
      friend.vel.x *= -0.85;
      this.registerImpact(impact, 0.9);
    ...
    if (friend.pos.y - radius < 0) {
      const impact = Math.abs(friend.vel.y);
      friend.pos.y = radius;
      friend.vel.y *= -0.82;
      this.registerImpact(impact, 1);
  ...
  handleScatterWalls(friend) {
    this.level.walls.forEach((rect) => {
      ...
      this.registerImpact(Math.abs(dot), rect.bounce || 0.96);
    });
  }
  ...
  handleBumpers(friend) {
    this.level.bumpers.forEach((rect) => {
      ...
      this.registerImpact(Math.abs(dot), (rect.bounce || 1.0) * 1.1);
    });
  }
    ...
    for (let i = 0; i < this.friends.length; i += 1) {
      ...
      for (let j = i + 1; j < this.friends.length; j += 1) {
        ...
        if (relSpeed < 0) {
          ...
          b.vel.x += impulseX;
          b.vel.y += impulseY;
          this.registerImpact(-relSpeed, 1.1);
        }
